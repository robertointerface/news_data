...
class ConvertToExcel(APIView):
    """
    Recieve JSON data and transform it into an excel file with the corresponding data by
    using 'xlwt' library.
    """
    ...
    def post(self, request, format=None):
        """
        Post request, receive JSON data and transform it into an excel file with data on it.

        @param
            request: http.request object with JSON object in the request body/data
        @return
            On success - excel file with corresponding data.
        ...
        try:
            ...
            self._response['Content-Disposition'] = 'attachment; filename="{file_name}"'.format(
                file_name=self._file_title)
            ...
            work_sheet.write(self.__message_row,
                             self.__message_col,
                             self._topic, self._head_row_style)

            work_sheet.write(self.__start_row,
                             self.__start_column,
                             'countries',
                             self._head_row_style)

            # create table head composed of the data times
            for i, column in enumerate(self._times):
                work_sheet.write(self.__start_row,
                                 ((self.__start_column + 1) + i),
                                 column, self._head_row_style)
                work_sheet.col(i).width = 4 * 256
            # Insert data into table
            for i, row in enumerate(request_data['result']):
                work_sheet.write((self.__start_row + 1) + i,
                                 self.__start_column, row['name'],
                                 self._data_row_style)
                for x, value in enumerate(row['values']):
                    work_sheet.write((self.__start_row + 1) + i,
                                     (self.__start_column + 1) + x,
                                     value,
                                     self._data_row_style)
    ...
    def built_title(self, extension):
        ...
        @params
            extension - string indicating the file extension, 'xls' or 'pdf'.
    ...
    def create_columns(self, result):
        """Iterate over result and create an array of countries in result

            @params
                result - JSON object used to create the excel file
            @return
                array with countries names
        """
